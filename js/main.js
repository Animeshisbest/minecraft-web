...
function upsertMesh(x, y, z, type) {
  const k = keyOf(x, y, z);
  removeMesh(k);
  const mesh = createBlock(type, x, y, z);
  if (!mesh) return;
  meshes.set(k, mesh);
  blockGroup.add(mesh);
}
...
function buildHotbar() {
  ...
  hotbar.forEach((numericId) => {
    ...
    swatch.style.backgroundImage = `url(${getBlockPreview(numericId)})`;
    ...
    .map(([id, block]) => {
      ...
      return `
        <div class="inv-slot ${activeClass} ${emptyClass}" data-id="${id}" title="${id}: ${block.name}">
          ...
          <div class="swatch" style="background-image:url('${getBlockPreview(id)}')"></div>
